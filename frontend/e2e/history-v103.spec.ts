...
test('favorites work in catalog and original decision template without navigation',async({page,request},info)=>{
 ...
 await expect(frame.locator('[data-favorite="'+code+'"]').first()).toHaveAttribute('aria-pressed','true')
 await frame.locator('[data-favorite="'+code+'"]').first().click()
 await expect(frame.locator('[data-favorite="'+code+'"]').first()).toHaveAttribute('aria-pressed','false')
...
test('human review saves privately without a model; AI connection steps are visible',async({page},info)=>{
 ...
 await page.goto('/ai');await page.getByRole('button',{name:'本地 Codex',exact:true}).click();await expect(page.getByTestId('ai-setup-guide')).toContainText('CODEX_HOME')
 await page.getByRole('button',{name:'模型 API',exact:true}).click()
 await expect(page.getByTestId('ai-setup-guide')).toContainText('同一份个人配置')
