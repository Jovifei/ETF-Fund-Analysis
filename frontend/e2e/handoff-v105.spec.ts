...
test('AI setup exposes interactive mode cards and never calls a model while switching steps', async ({page}) => {
  ...
  await expect(guide.getByRole('tab',{name:'模型 API'})).toHaveAttribute('aria-selected','true')
  await guide.getByRole('tab',{name:'本地 Codex'}).click()
  await expect(guide.getByTestId('ai-mode-codex')).toBeVisible()
  await guide.getByRole('tab',{name:'不用 AI'}).click()
