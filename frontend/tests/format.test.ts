...
import { direction, num, pct, frequency, forecastLabel, relativeAge, stamp } from '../src/lib/format'
describe('display contracts', () => {
  ...
  it('shares timestamp parsing with news age filters and fails closed for future or missing times', () => {
    const now = Date.parse('2026-09-10T00:00:00Z')
    expect(relativeAge('2026-09-10 07:00:00', now)).toEqual({ valid: true, future: false, hours: 1 })
    expect(relativeAge('2026-09-10T01:00:00Z', now)).toEqual({ valid: true, future: true, hours: null })
    expect(relativeAge(undefined, now)).toEqual({ valid: false, future: false, hours: null })
  })
})
