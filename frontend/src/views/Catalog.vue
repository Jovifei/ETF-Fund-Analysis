...
const route=useRoute(),text=ref(String(route.query.q??'')),search=ref(text.value),q=useQuery<{items:SearchItem[]}>(()=>'/api/search/instruments?limit=100&q='+encodeURIComponent(search.value))
watch(()=>route.query.q,value=>{text.value=String(value??'');search.value=text.value})
</script>
