...
async function remove(id:number){if(!window.confirm('只取消自选，不会删除持仓。继续吗？'))return;busy.value=id;try{await api('/api/watchlist/entries/'+id,{method:'DELETE'});await q.reload();await favorites.reload()}catch(e){message.value=errorText(e)}finally{busy.value=null}}
</script>
