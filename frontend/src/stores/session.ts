...
export const useSession = defineStore('session', () => {
  ...
  async function login(identifierInput: string, password: string) { error.value = ''; feedback.value=''; await api('/api/auth/login', { method: 'POST', body: { identifier: identifierInput, password } }); generation.value++; await load() }
  async function logout() { await api('/api/auth/logout', { method: 'POST' }); clear() }
  return { feedback, ready, authenticated, identifier, role, error, generation, clear, load, login, logout }
})
