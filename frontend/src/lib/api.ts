...
let sessionEpoch = 0
export function abortAllRequests() { sessionEpoch++; for (const controller of requests) controller.abort(); requests.clear() }
export function csrfToken(cookie?: string): string {
  if (cookie === undefined) { try { cookie = document.cookie } catch { return '' } }
...
export async function api<T>(path: string, options: { method?: string; body?: unknown; signal?: AbortSignal; timeout?: number; blob?: boolean } = {}): Promise<T> {
  ...
  const assertCurrent = () => { if (controller.signal.aborted || epoch !== sessionEpoch) throw new DOMException('Request is no longer current', 'AbortError') }
  ...
  try {
    ...
    const value = options.blob ? await response.blob() : response.status === 204 ? undefined : await response.json()
    assertCurrent()
    return value as T
