...
python bridge/etf_agent_bridge.py --root $root work --model YOUR_MODEL_ID --max-jobs 1 --max-minutes 10 --approve-execution`
</script>
...
<div v-else-if="mode==='codex'" data-testid="ai-mode-codex" role="tabpanel"><h3>本地 Codex · 隔离执行</h3><ol class="ai-step-list"><li>本机安装官方 Codex CLI 和本项目依赖；使用独立目录，不能复制全局 auth.json。步骤见 <code>docs/LOCAL_CODEX_BRIDGE.md</code>。</li><li>API服务私人配置启用 <code>WORKSPACE_BRIDGE_ENABLED=true</code>，重启服务；网页“设置 → 隔离研究设备”生成一次性配对码。</li><li>在本机新 PowerShell 执行下方命令。官方登录由你本人完成；pair时在终端输入配对码。服务不在8082时改成你的实际HTTPS地址，不要关闭证书验证。</li><li>网页建立研究任务，核对模型与证据后才确认费用并执行一次work。无效结果会标失败；有效结果上传失败只重传，不自动再次调用模型。报告回传后在研究任务查看和审核。doctor仅检查桥接，不等于模型账号已登录。</li></ol><pre class="report-text">{{powershell}}</pre><p>本项目会检查 Codex 版本与工具隔离。出现 unreviewed_codex_version 时按 bridge 文档审查版本，不删除门禁。每天自动运行应在一次真实任务验证后另行启用。</p><p>Vibe 的深度研究属于独立可选适配，先按 <code>docs/LOCAL_ACCEPTANCE_V103.md</code> 与实际安装脚本 --help 固定版本隔离验收，不直接给它生产数据库凭据。</p></div>
