...
<template><section class="card section" data-testid="ai-setup-guide"><div class="card-header"><h2>如何开始 AI 研究</h2></div><div class="card-body"><div class="segmented ai-mode-cards" role="tablist" aria-label="AI连接方式"><button class="mode-card" role="tab" :class="{active:mode==='api'}" :aria-selected="mode==='api'" :aria-pressed="mode==='api'" @click="mode='api'">模型 API<small>复用已保存配置</small></button><button class="mode-card" role="tab" :class="{active:mode==='codex'}" :aria-selected="mode==='codex'" :aria-pressed="mode==='codex'" @click="mode='codex'">本地 Codex<small>隔离设备执行</small></button><button class="mode-card" role="tab" :class="{active:mode==='manual'}" :aria-selected="mode==='manual'" :aria-pressed="mode==='manual'" @click="mode='manual'">不用 AI<small>保留确定性研究</small></button></div>
