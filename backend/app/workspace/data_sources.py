...
def inspect_sources(db, settings) -> dict:
    ...
    sources = [
        ...
        {"id": "tencent", "configured": True, "installed": ak, "deadline_seconds": settings.akshare_timeout_seconds,
         "status": "public_source_not_verified" if ak else "package_missing",
         "capabilities": {"daily": "stock_zh_a_hist_tx", "quote": "not_connected", "minutes": "not_connected"},
         "notes": ["日线量额按适配器合同统一为股/元", "公开源仍需同日证据覆盖；不提供实时资格"]},
