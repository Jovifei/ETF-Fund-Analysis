...
from app.workspace.data_sources import configuration
...
def status(db: DB, settings: Config, user: User):
    ...
    return {"app_version": settings.app_version, "workspace_version": "1.0.4", "strategy_version": strategy.get("version"), "indicator_version": strategy.get("indicator_version"), "forecast_version": strategy.get("forecast_version"), "market_provider": configuration(db, settings)["effective_provider"], "ui_enabled": cfg.ui_enabled, "bridge_enabled": cfg.bridge_enabled, "daily_review_enabled": cfg.daily_review_enabled, "worker": heartbeat.settings_json if heartbeat else None, "catalog_count": db.scalar(select(func.count()).select_from(Instrument).where(Instrument.kind.in_(("ETF", "LOF")))), "tracked_count": db.scalar(select(func.count()).select_from(Instrument).where(Instrument.enabled.is_(True))), "historical_1430_backtest": "not_qualified", "api_key_configuration": "encrypted_opt_in_profiles", "automatic_orders": False}


@router.get("/data-sources")
def data_sources(db: DB, settings: Config, user: Admin):
    from app.workspace.data_sources import inspect_sources
    return inspect_sources(db, settings)
...
@router.post("/external-research/preview")
def preview_external(payload: ExternalPacket, user: User):
    return preview(payload)


@router.post("/external-research/import", status_code=201)
def import_external(payload: ExternalImport, db: DB, user: User):
    row, created = import_packet(db, payload.packet, payload.packet_hash, user.id if user else None)
    db.commit()
    return {"job": jobs.job_view(row, detail=True), "created": created, "model_called": False}


@router.get("/research-styles")
def research_styles(user: User):
    from app.core.config import PROJECT_ROOT
    from app.workspace.external_research import strict_json
    source = PROJECT_ROOT / "config" / "research_styles.json"
    payload = strict_json(source.read_text(encoding="utf-8"))
    return {**payload, "content_hash": content_hash(payload), "actionable": False}
