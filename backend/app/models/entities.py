...
class IndicatorSnapshot(Base):
    ...
    reproducibility_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
...
class ForecastSnapshot(Base):
    ...
    terminal_price_q10: Mapped[float | None] = mapped_column(Float)
    terminal_price_q50: Mapped[float | None] = mapped_column(Float)
    terminal_price_q90: Mapped[float | None] = mapped_column(Float)
    path_low_price_q10: Mapped[float | None] = mapped_column(Float)
    path_low_price_q50: Mapped[float | None] = mapped_column(Float)
    path_low_price_q90: Mapped[float | None] = mapped_column(Float)
    path_high_price_q10: Mapped[float | None] = mapped_column(Float)
    path_high_price_q50: Mapped[float | None] = mapped_column(Float)
    path_high_price_q90: Mapped[float | None] = mapped_column(Float)
    corridor_position: Mapped[float | None] = mapped_column(Float)
    support_touch_probability: Mapped[float | None] = mapped_column(Float)
    resistance_touch_probability: Mapped[float | None] = mapped_column(Float)
    interval_method: Mapped[str | None] = mapped_column(String(64))
...
class CalibrationProfile(Base):
    """校准候选档案：calibrate_forecasts 只创建 candidate，绝不自动批准。

    状态机：candidate → approved/rejected，转换仅允许人工通过 API/CLI 显式触发；
    任何情况下本表的存在都不改变 ForecastSnapshot.calibration_status。
    """

    __tablename__ = "calibration_profiles"
    __table_args__ = (
        UniqueConstraint("validation_content_hash", name="uq_calibration_profiles_validation_hash"),
        CheckConstraint(
    ...
    validation_run_id: Mapped[str] = mapped_column(String(64))
...
class SignalSnapshot(Base):
    ...
    indicator_version: Mapped[str] = mapped_column(String(64))
    forecast_version: Mapped[str] = mapped_column(String(64))
