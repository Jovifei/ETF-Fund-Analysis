...
应用/发行包版本为 `0.6.0`。本版本加入多模型分析网关、六项市场上下文和私有持仓 OCR 流程的文档/验证闭环，但不升级 `config/strategy.json` 中的策略、指标、预测或回测版本（当前策略仍含 `signal-v0.4.0`）。当前结论必须写作本地/Mock 已验证，不能写作生产就绪、实时、calibrated 或投资建议。
