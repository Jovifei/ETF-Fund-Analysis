# v105 接收与生产交接（2026-09-11）

## v105 当前接力（2026-09-11）

阶段性全面审核（2026-09-12）已归档到 `docs/CODE_AUDIT_BLOCKERS_20260912.md`。后续接力先处理报告中的 P1 数据资格、结算调度、任务状态、CI 和 Codex Bridge 阻碍，再重新做生产资格审核；本次审核没有改业务代码。

v105 已在独立分支 `codex/v105-handoff-local-20260910` 接收并完成本地复测，生产当前运行缓存修复提交 `406cad0` 的独立源目录 `deploy-v105-406cad0`；API、worker healthy，scheduler running，v1.0.4 源目录、量能修复源目录、镜像和精确回滚 Compose 保留。

本地真实副本运行于 `http://127.0.0.1:8084`，两只 ETF 日线已到 2026-09-10，三指数 OHLC 缓存已记录；本地副本仍保留旧 v101 price-only 证据，生产已通过 v102 单位校验重算 35 个标的。中证全指上下文、实时资格边界保持阻断；SQLite scheduler 与 worker 并发写锁是本地剩余问题，详见两份 v105 收据。

本包基线为 `46c713d4a7f6f247461ec9b075948f25c6741df7`；首次部署代码 HEAD 为 `409a5e5`，量能修复提交为 `208858e`，缓存修复提交为 `406cad0`，当前文档收口随其后，应用包版本仍为1.0.4。接收分支已推送远端，`main` 未合并、标签未移动。

先读 `docs/LOCAL_ACCEPTANCE_RECEIPT_V105_20260911.md` 和 `docs/PRODUCTION_DEPLOYMENT_RECEIPT_V105_20260911.md`。原表比较、图表虚线/全屏、AI交互、真实副本和生产切换已有证据；Windows DPAPI、专用 PostgreSQL 条件套件、完整缠论、OCR/Vibe/真人模型、因子资格和自动云地同步仍保持未通过边界。平衡刷新默认关闭。以下历史记录原样保留，不覆盖本节当前生产事实。
