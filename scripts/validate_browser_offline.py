...
def main():
    ...
    with TemporaryDirectory(prefix='etf-offline-validation-') as temp:
        ...
        with TestClient(app) as client,sync_playwright() as pw:
            ...
            page.evaluate("window.validationRouter.push('/matrix')")
            expect(page.get_by_role('heading',name='ETF 指标总表',exact=True)).to_be_visible()
            expect(page.get_by_role('columnheader',name='MA 均线',exact=True)).to_be_visible()
            expect(page.get_by_role('columnheader',name='TD9',exact=True)).to_be_visible()
            expect(page.locator('tbody tr').first).to_be_visible()
            page.screenshot(path=str(out/'v101-matrix.png'),full_page=True)
            checks.append('v101 ETF matrix reuses canonical backend indicators')
            page.evaluate("window.validationRouter.push('/settings')")
            expect(page.get_by_role('heading',name='后端数据接入',exact=True)).to_be_visible()
            expect(page.get_by_test_id('data-connections')).to_contain_text('未配置 Token')
            page.screenshot(path=str(out/'v101-data-connections.png'),full_page=True)
            page.get_by_role('button',name='更新跟踪池行情',exact=True).click()
            expect(page.get_by_role('status').filter(has_text='后台任务已排队')).to_be_visible()
            pending=client.get('/api/workspace/data-jobs').json()
            assert any(item['task']=='prices' and item['status']=='queued' for item in pending['items'])
            checks.append('v101 source status and price task queue do not call a provider from the browser')
            ...
            # Actual classic HTML/JS consumes the same ASGI snapshot; no URL navigation.
            import re
            classic=browser.new_page(viewport={'width':1800,'height':1100},device_scale_factor=1)
            classic.on('pageerror',lambda e:console_errors.append(str(e)))
            classic.expose_binding('__asgiFetch',exchange)
            markup=(root/'backend/app/static/decision_board_workbuddy.html').read_text()
            markup=re.sub(r'<link[^>]*rel="stylesheet"[^>]*>', '', markup)
            markup=re.sub(r'<script[^>]*src="[^"]*"[^>]*></script>', '', markup)
            classic.set_content(markup)
            classic.add_style_tag(content=(root/'backend/app/static/decision_board_workbuddy.css').read_text())
            classic.add_script_tag(content="""window.fetch=async (path,options={})=>{
              const r=await window.__asgiFetch({path:String(path),method:options.method||'GET',headers:options.headers||{},body:options.body});
              return new Response(Uint8Array.from(atob(r.body),c=>c.charCodeAt(0)),{status:r.status,headers:r.headers});
            };""")
            classic.add_script_tag(content=(root/'backend/app/static/decision_board_workbuddy.js').read_text())
            classic.evaluate("document.dispatchEvent(new Event('DOMContentLoaded'))")
            expect(classic.locator('.decision-data-row').first).to_be_visible(timeout=15000)
            classic.screenshot(path=str(out/'v101-classic-matrix.png'),full_page=True)
            checks.append('preserved original classic ETF matrix renders the same canonical ASGI snapshot')
